...
where USERNAME_ = 'E_SELIM'

select * from BASKETDETAIL

where ID = 4

-- Bu kod boyle yazinca çalýþýyor--
select U.USERNAME_, U.GENDER, ADDRES.ADDRESSTEXT, CITY.CITY, TOWN.TOWN, DISTRICT.DISTRICT,BASKETDETAIL.PRICE,BASKETDETAIL.AMOUNT, 
BASKETDETAIL.TOTAL, ORDER_.STATUS_,ORDERDETAIL.TOTALPRICE as Toplam_Odenen_Tutar, ORDER_.BASKETID from USER_ U


INNER JOIN ADDRES on ADDRES.ID = U.ID
JOIN CITY on CITY.ID = ADDRES.CITYID
JOIN TOWN on TOWN.ID = ADDRES.TOWNID
JOIN DISTRICT on DISTRICT.ID = ADDRES.DISTRICTID
JOIN BASKETDETAIL on BASKETDETAIL.ID = U.ID
JOIN ORDER_ on ORDER_.ID = BASKETDETAIL.BASKETID
JOIN ORDERDETAIL on ORDERDETAIL.ORDERID = BASKETDETAIL.BASKETID

-- DISTINCT ÝLE ÝLGÝLÝ BÝR DENEY -- 
select DISTINCT(U.USERNAME_), U.GENDER, ADDRES.ADDRESSTEXT, CITY.CITY, TOWN.TOWN, DISTRICT.DISTRICT,BASKETDETAIL.PRICE,BASKETDETAIL.AMOUNT, 
BASKETDETAIL.TOTAL, ORDER_.STATUS_,ORDERDETAIL.TOTALPRICE as Toplam_Odenen_Tutar, ORDER_.BASKETID from USER_ U
...
-- fakat aþaðýdaki gibi sum ekleyince çalýþmýyor bunu araþtýrýyoruz þuan--

select DISTINCT(U.USERNAME_), U.GENDER, ADDRES.ADDRESSTEXT, CITY.CITY, TOWN.TOWN, DISTRICT.DISTRICT,BASKETDETAIL.PRICE,ORDER_.STATUS_, 
sum(BASKETDETAIL.TOTAL) as Toplam_Odenen_Tutar, ORDER_.BASKETID, ORDER_.BASKETID from USER_ U
...
order by sum(BASKETDETAIL.TOTAL) desc

-- ÖNEMLÝ BÝR NOT!!! Yukarýdaki kodda TOTALPRÝCE'YÝ BULAMIYOR ÇÜNKÜ TOTALPRÝCE'NÝN BAÐLI OLDUÐU BÝR TABLO YOK

select sum(BASKETDETAIL.TOTAL) as Toplam_Odenen_Tutar,USER_.NAMESURNAME , USER_.GENDER, ADDRES.ADDRESSTEXT, CITY.CITY, TOWN.TOWN, DISTRICT.DISTRICT,BASKETDETAIL.PRICE,ORDER_.STATUS_,
ORDER_.BASKETID, ORDER_.BASKETID from USER_

INNER JOIN ADDRES on ADDRES.ID = USER_.ID
JOIN CITY on CITY.ID = USER_.ID
JOIN TOWN on TOWN.ID = USER_.ID
JOIN DISTRICT on DISTRICT.ID = USER_.ID
JOIN BASKETDETAIL on BASKETDETAIL.ID = USER_.ID
JOIN ORDER_ on ORDER_.ID = BASKETDETAIL.BASKETID
JOIN ORDERDETAIL on ORDERDETAIL.ORDERID = BASKETDETAIL.BASKETID
